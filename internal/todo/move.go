...
// MoveToTop relocates the item at idx to the top of the target section.
func (l *List) MoveToTop(s Schema, idx int, toKey string) error {
	if err := l.inRange(idx); err != nil {
		return err
	}
	if _, ok := s.byKey(toKey); !ok {
	...
	l.Items[idx].Section = toKey
	l.Items[idx].Order = -1 // sorts to the top after Normalize
	return nil
}
